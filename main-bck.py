...
if services_model >= 1:
    ...
    async def get_document_count(collection_name: str):
        ...
        """
        log.logger.debug(
            'Access \'/_collection/{collection_name}\' : run in get_document_count(), input collection_name: [%s]' % collection_name)
    ...
    @app.get(prefix + "/_sysdef/sysdeftest/{syscol_name}",
             tags=["System Define - Collection Level"],
             summary="Retrieve system definitions count.",
             description="",
             )
    async def get_sysdef_test_count(syscol_name: str, current_user_role: bool = True if services_model >= 1 else Depends(security.get_super_permission) ):
        """
                This describes the document count
        """
        log.logger.debug(
