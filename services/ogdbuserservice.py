...
from models.ogdbuser import ogdbuser
...
class ogdbuserService(object):
    def __init__(self):
        self._modelName = 'ogdbuser'
    ...
    def getall_ogdbuser(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(ogdbuser).limit(cfg['Query_Config'].query_limit_upset)
                return results.all()
    ...
    def get_ogdbuser_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.execute('select count(*) from ogdbuser')
    ...
    def create_ogdbuser(self, customer):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                ...
                pks = ogdbuser.getPrimaryKeys(ogdbuser)
                returnjson = {}
                for pk in pks:
    ...
    def batch_create_ogdbuser_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_ogdbuser(newentity)
    ...
    def get_ogdbuser_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(ogdbuser).where(eval(idstr))
                result = session.exec(statement).one()
                #log.logger.debug('get_ogdbuser_byid() result is : %s' % result)
                return result
        except Exception as e:
    ...
    def update_ogdbuser(self, customer):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(customer)
    ...
    def update_ogdbuser_byjson(self, updatejson):
        log.logger.debug('The update JSON is: %s' % updatejson)
        pks = ogdbuser.getPrimaryKeys(ogdbuser)
        statement = select(ogdbuser)
        for pk in pks:
            statement = statement.where(eval("ogdbuser."+ pk + "==" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
    ...
    def batch_update_ogdbuser_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_ogdbuser_byjson(bdata)
    ...
    def delete_ogdbuser_byid(self, idstr):
        try:
            ...
            with Session(engine) as session:
                statement = select(ogdbuser).where(eval(idstr))
                result = session.exec(statement).one()
                log.logger.debug('delete_ogdbuser_byid() result is : %s' % result)
    ...
    def query_ogdbuser(self,querystr):
        if toolkit.validQueryJson(querystr):
            ...
            if len(queryfields.split(',')) == 1:
                ...
                statement = select(from_obj=ogdbuser, columns=eval(queryfields))
            #add distinct
            ...
            try:
                ...
                with Session(engine) as session:
                    ...
                    if include_count | count_only:
                        pks = ogdbuser.getPrimaryKeys(ogdbuser)
                        qfields = "ogdbuser." + ",ogdbuser.".join(tuple(pks))
                    ...
                    if count_only:
                        ...
                        # log.logger.debug('query_ogdbuser() result is : %s' % result)
                        if include_count:
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.logger.debug('query_ogdbuser() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
