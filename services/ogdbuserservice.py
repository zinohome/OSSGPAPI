...
class ogdbuserService(object):
    ...
    def get_ogdbuser_count(self):
        try:
            ...
            with Session(engine) as session:
                ...
                stmt = text("select count(" + pks[0] + ") as rowcount from ogdbuser")
    ...
    def get_ogdbuser_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(ogdbuser).where(text(idstr))
    ...
    def update_ogdbuser_byjson(self, updatejson):
        ...
        for pk in pks:
            if ogdbuser.getpkqmneed(ogdbuser,pk):
                statement = statement.where(text("ogdbuser." + pk + "='" + str(updatejson[pk])+"'"))
            else:
                statement = statement.where(text("ogdbuser."+ pk + "=" +str(updatejson[pk])))
        try:
            engine = dbengine.DBEngine().connect()
