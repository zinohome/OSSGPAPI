...
from models.ogdbconnect import ogdbconnect
...
class ogdbconnectService(object):
    def __init__(self):
        self._modelName = 'ogdbconnect'

    def new_model(self, modeljson):
        newmodel = ogdbconnect()
    ...
    def getall_ogdbconnect(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                results = session.query(ogdbconnect).limit(cfg['Query_Config'].query_limit_upset)
                return results.all()
        except Exception as e:
            log.logger.error('Exception at getall_ogdbconnect(): %s ' % e)
    ...
    def get_ogdbconnect_count(self):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
    ...
    def create_ogdbconnect(self, customer):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.logger.error('Exception at create_ogdbconnect(): %s ' % e)
            if cfg['Application_Config'].app_exception_detail:
                traceback.print_exc()
            return None
        finally:
    ...
    def batch_create_ogdbconnect_byjson(self, jsonstr):
        ...
        try:
            for bdata in batchdata:
                ...
                newid = self.create_ogdbconnect(newentity)
                returnjson['ids'].append(newid)
                returnjson['data'].append(newentity)
            return returnjson
        except Exception as e:
            log.logger.error('Exception at batch_create_ogdbconnect(): %s ' % e)
            if cfg['Application_Config'].app_exception_detail:
                traceback.print_exc()
            return None

    def get_ogdbconnect_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
            ...
            log.logger.error('Exception at get_ogdbconnect_byid(): %s ' % e)
            if cfg['Application_Config'].app_exception_detail:
                traceback.print_exc()
            return None
        finally:
    ...
    def update_ogdbconnect(self, customer):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                session.add(customer)
            ...
            log.logger.error('Exception at update_ogdbconnect(): %s ' % e)
            if cfg['Application_Config'].app_exception_detail:
                traceback.print_exc()
            return None
        finally:
    ...
    def update_ogdbconnect_byjson(self, updatejson):
    ...
    def batch_update_ogdbconnect_byjson(self, jsonstr):
        batchjson = json.loads(jsonstr)
        batchdata = batchjson['data'] if 'data' in batchjson else None
        returnjson = {"data": []}
        try:
            for bdata in batchdata:
                updentity = self.update_ogdbconnect_byjson(bdata)
                returnjson['data'].append(updentity)
            return returnjson
        except Exception as e:
            log.logger.error('Exception at batch_update_ogdbconnect_byjson(): %s ' % e)
            if cfg['Application_Config'].app_exception_detail:
                traceback.print_exc()
            return None

    def delete_ogdbconnect_byid(self, idstr):
        try:
            engine = dbengine.DBEngine().connect()
            with Session(engine) as session:
                statement = select(ogdbconnect).where(text(idstr))
            ...
            log.logger.error('Exception at delete_ogdbconnect_byid(): %s ' % e)
            if cfg['Application_Config'].app_exception_detail:
                traceback.print_exc()
            return False
        finally:
                ...
                with Session(engine) as session:
                    ...
                    if count_only:
                        ...
                        if include_count:
                            ...
                            returnjson['data'] = data
                            returnjson['rawdata'] = rawdata
                    # log.logger.debug('query_ogdbconnect() returndict is : %s' % returndict)
                    return returnjson
            except Exception as e:
                log.logger.error('Exception at query_ogdbconnect(): %s ' % e)
                if cfg['Application_Config'].app_exception_detail:
                    traceback.print_exc()
                return None
            finally:
